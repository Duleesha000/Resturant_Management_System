...
struct admin adminCredentials = {"admin", "password"};
...
void admin()
{
   printf("\n\t\t\t\t\t   ----------------------------------------------\n");
    ...
    while (1)
    {
        char ch = _getch(); // Read a character without echoing to the console
        ...
        else if (ch == 8 && i > 0) // Handle backspace
        {
            i--;
            password[i] = '\0';
            printf("\b \b"); // Move the cursor back and overwrite the character
        }
        else if (i < sizeof(password) - 1) // Store the character in the password array
        {
            password[i] = ch;
            i++;
            printf("*"); // Print a '*' instead of the actual character
        }
    }
...
int main()
{
    heada = createadmin(heada,1,"Vegitable Soup",500);
    heada = createadmin(heada,2,"Nasiguran     ",850);
    heada = createadmin(heada,3,"Biriani       ",1200);
    heada = createadmin(heada,4,"Fried Rice    ",1000);
    heada = createadmin(heada,5,"Noodles       ",750);
